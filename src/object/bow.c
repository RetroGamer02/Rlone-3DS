...
const tier_weight_t	bow_tier[] = {
	{ OBJ_BOW_LEGEND,   1 },
	{   OBJ_BOW_RARE,  99 },
	{ OBJ_BOW_COMMON, 900 }
};
const	int			bow_tier_len = 3;
#define	BOW_ROLL	1000

enum object
_bow_roll_type()
{
	int roll = ROLL(BOW_ROLL),
		next_roll;
	int	floor = object_get_floor();
	int i;

	while (floor-- > 0) {
		next_roll = ROLL(BOW_ROLL);
		if (next_roll > roll)
			roll = next_roll;
	}

	for (i = 0; i < bow_tier_len; i++) {
		if (roll >= (BOW_ROLL - bow_tier[i].weight))
			return bow_tier[i].type;
	}

	/* Return the lowest option. */
	return bow_tier[bow_tier_len - 1].type;
}

void
_bow_roll_values(object_t *o, int dmg, int pen)
{
...
object_bow_init(object_t *o)
{
	...
	case OBJ_BOW_RARE:
		...
		break;
	case OBJ_BOW_COMMON:
		o->ent = ENT_BOW_COMMON;
		max_dmg = 10;
		max_ap = 20;
		break;
	case OBJ_BOW:
	default:
		/* Generate what bow_tier the bow is. */
		o->type = _bow_roll_type();
		object_bow_init(o);
		return;
	}

	while (rolls-- > 0)
		_bow_roll_values(o, max_dmg, max_ap);

	_bow_build_inspect(o);

#ifdef DEBUG
	debug("Bow generated: DMG %d-%d, AP %d-%d.\n", o->hp, o->hp_max, o->ac, o->ac_max);
#endif
}

void
object_bow_free(object_t *o)
{
